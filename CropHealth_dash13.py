...
with tab1:
    ...
    if analysis_type == "Team-Based":
        ...
        with col2:
            ...
            # 3D Surface Plot for Cumulative Data
            if "Nitrogen fertilizer" in plot_options and "Irrigation amounts" in plot_options:
                ...
                mean_ndvi = summary_table.mean(axis=1)
                ...
                fig_surface = go.Figure(data=[go.Surface(
                    x=surface_data["Cumulative Nitrogen"],
                    y=surface_data["Cumulative Irrigation"],
                    z=surface_data["Mean NDVI"],
                    colorscale="Viridis",
                )])

                fig_surface.update_layout(
                    title="3D Surface Plot of NDVI vs. Cumulative Nitrogen and Irrigation",
                    scene=dict(
                        xaxis_title="Cumulative Nitrogen (lbs/acre)",
                        yaxis_title="Cumulative Irrigation (inches)",
                        zaxis_title="Mean NDVI"
                    ),
                    width=800,
                    height=600
                )
